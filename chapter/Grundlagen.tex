...
Im folgenden Abschnitt werden Funktionsweise und Besonderheiten der verwendeten Tech-
nologien beschrieben.
...
Der Charakter dieser Datenbank wird durch mehrere Punkte bestimmt. Während relationale Datenbanksysteme auf Tabellen aufbauen, basiert MongoDB auf sogenannten Collections, einer Menge von Dokumenten\footnote{Vgl. \cite{mongoDef} S. 7}.  Dieser dokumentenorientierte Ansatz, ersetzt das Konzept einer "'Zeile"' in relationalen Datenbanken mit dem flexiblerem Dokumentenmodell und ermöglicht es so, in einem einzigen Datensatz komplexe hierarchische Beziehungen darzustellen. Das Datenformat für Dokumente ist BSON (Binaray JSON), eine Erweiterung von JSON um einige binäre Typen. Zu den unterstützten Datentypen gehören also neben \textit{Strings}, \textit{Numbers}, \textit{Objects}, \textit{Arrays}, \textit{Null}, \textit{Boolean} auch \textit{Date} Objekte, \textit{Reguläre Ausdrücke}, \textit{ObjectIDs} und \textit{eingebettete Dokumente}\footnote{Vgl. \cite{mongoDef} S. 17ff}. %Ein Dokument könnte demnach so aussehen: \\
%\begin{listing}[H]
%\centering
%\inputminted[linenos, mathescape, numbersep=5pt,frame=lines, framesep=2mm,tabsize=4,fontsize=\small]{javascript}{./code/pageDatabaseobject.js}
%\caption[Dokument]{MongoDB Dokument}
%\label{fig:callback}
%\end{listing} \\
In relationalen Datenbanken haben Entitäten ein striktes Schema, was bedeutet dass alle Reihen einer Tabelle die selben Spalten besitzen muss. In MongoDB gibt es keine vordefinierten Schemata: Schlüssel und Werte eines Dokuments sind nicht von festen Typen oder Größen. Ohne festes Schema ist das, Hinzufügen oder Entfernen von Feldern einfacher und im Allgemeinen auch schneller.
